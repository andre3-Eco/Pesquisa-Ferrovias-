...
# Tratamento: Densidade Buffer (Real) | Instrumento: Densidade Buffer (Sintética)
# Outputs: PIB Total, PIB Setorial, Urbanização, IDH Decomposto
# ==============================================================================
...
# Identificar e remover colunas sobrepostas para evitar sufixos .x e .y
...
outcomes_map <- tibble(
  ...
  needs_log = c(
    TRUE, TRUE, TRUE, TRUE,       # PIB -> log
    FALSE, FALSE,                 # Urbanização -> Nível
    FALSE, FALSE, FALSE, FALSE    # IDH -> Nível
  )
)
...
for (i in seq_len(nrow(outcomes_map))) {
  ...
  if (n_years > 0) {
    cat(sprintf("   • %s (%s): %d anos [%d-%d]\n",
                row$prefixo, row$escopo, n_years,
                min(row$anos_disponíveis[[1]]), max(row$anos_disponíveis[[1]])))
  }
}
cat("\n")
...
treatment_years <- sort(as.integer(sub("densidade_buffer_real_", "", dens_real_cols)))
...
rodar_2sls <- function(df, endo_var, inst_var, outcome_col, outcome_nome, 
  ...
  tryCatch({
    ...
    # Proteção: Lida com versões do fixest que usam ou não usam "fit_"
...
for (ano_trat in treatment_years) {
  ...
  if (contador_anos %% 5 == 0 || ano_trat == max(treatment_years)) {
...
output_file <- "03-resultados/csv/second_stage_multidimensional_results.csv"
