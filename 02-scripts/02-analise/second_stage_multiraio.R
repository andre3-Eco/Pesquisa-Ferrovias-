# ==============================================================================
# Etapa 21
# SECOND-STAGE IV (2SLS) – MÚLTIPLOS RAIOS DE BUFFER
# ==============================================================================
...
base <- base_main %>%
  ...
  # Remove TODAS as colunas antigas de densidade para evitar sufixos .x e .y
  select(-starts_with("densidade_")) %>%
  left_join(base_densidade, by = "code_amc") %>%
  # Removido: left_join(painel_pontas...) para evitar a explosão combinatória de observações!
...
# -------------------- 7. FUNÇÃO SECOND STAGE (CORRIGIDA) --------------------
# -------------------- 7. FUNÇÃO SECOND STAGE (SUPER-ROBUSTA) --------------------
rodar_2sls <- function(df, endo_var, inst_var, outcome_col, outcome_nome,
  ...
  # 1. Rodar modelo protegidamente
  mod <- tryCatch({
    feols(as.formula(formula_str), data = df, se = "hetero", warn = FALSE)
  }, error = function(e) return(NULL))
  
  if (is.null(mod)) return(NULL)
  
  # 2. Extrair tabela de coeficientes
  ct <- tryCatch({ as.data.frame(summary(mod)$coeftable) }, error = function(e) return(NULL))
  if (is.null(ct) || nrow(ct) == 0) return(NULL)
  
  # 3. Busca Robusta de Nome: captura se for "nome" ou "fit_nome"
  nome_coef <- grep(endo_var, rownames(ct), value = TRUE)
  
  # Se o fixest omitiu a variável por colinearidade (comum em recortes pequenos)
  if (length(nome_coef) == 0) return(NULL) 
  
  nome_coef <- nome_coef[1] # Garante pegar o nome correspondente
  ...
  # 4. Extração isolada do F-Stat (não anula o resto se falhar)
  f_stat <- NA_real_
  try({
  ...
  # 5. Extração isolada do R2 (não anula o resto se falhar)
  r2_val <- NA_real_
  try({
  ...
  tibble(
    ...
    R2_2estagio   = r2_val,
    n_observacoes = mod$nobs, # Usa o N real reportado pelo próprio modelo
...
# -------------------- 8. LOOP PRINCIPAL --------------------
cat("Etapa 3: Rodando regressões do Segundo Estágio (2SLS)...\n")
...
# Garantir que a pasta exista antes de tentar salvar
output_dir <- "03-resultados/csv"
if (!dir.exists(output_dir)) dir.create(output_dir, recursive = TRUE)

output_file <- paste0(output_dir, "/second_stage_multiraio.csv")
