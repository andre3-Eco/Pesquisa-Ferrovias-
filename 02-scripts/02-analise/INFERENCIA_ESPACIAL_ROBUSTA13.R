# ==============================================================================
# Etapa 13
# INFERÊNCIA ESPACIAL ROBUSTA: ERROS DE CONLEY + CLUSTERIZAÇÃO (OTIMIZADO)
# Instrumentos: densidade_buffer_sintetica_YYYY → densidade_buffer_real_YYYY
# Outcomes: Multidimensional (PIB, População, Urbanização, IDH)
...
# Evita duplicação de colunas (as infames .x e .y)
...
# Criação de cluster espacial amplo (caso falte variável natural de cluster)
...
anos_para_testar <- c(1996, 2003) # Evitar 2010 se o tratamento não atinge esse ano

# Ampliando o leque analítico
...
# ==============================================================================
# 5. FUNÇÃO DE ESTIMAÇÃO ESPACIAL (Com Proteção Fixest)
# ==============================================================================
...
rodar_2sls_espacial <- function(df, ano_trat, outcome_var, tipo_se = "hetero", cluster_var = "code_microrregiao") {
  ...
  tryCatch({
    ...
    # Busca robusta do coeficiente no coeftable (Resolve o bug do fixest)
...
write_csv(df_resultados, "03-resultados/csv/resultados_inferencia_espacial_robusta.csv")
...
p_se <- ggplot(df_grafico, aes(x = tipo_inferencia, y = se, group = factor(ano_trat), color = factor(ano_trat))) +
  ...
  labs(
    ...
    x = "Método de Clusterização / Conley", y = "Erro-Padrão (SE)", color = "Ano Tratamento"
  ) +
  theme_minimal(base_size = 11) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), strip.text = element_text(face = "bold"))

ggsave("03-resultados/graficos/inferencia_espacial_se_painel.png", p_se, width = 10, height = 7, dpi = 300)
...
p_pv <- ggplot(df_grafico, aes(x = tipo_inferencia, y = p_valor, group = factor(ano_trat), color = factor(ano_trat))) +
  ...
  labs(
    title = "Sensibilidade do P-Valor à Correlação Espacial",
    x = "Método", y = "P-valor", color = "Ano Tratamento"
  ) +
  theme_minimal(base_size = 11) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), strip.text = element_text(face = "bold"))

ggsave("03-resultados/graficos/inferencia_espacial_pvalor_painel.png", p_pv, width = 10, height = 7, dpi = 300)
