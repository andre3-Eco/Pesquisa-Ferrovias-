# ==============================================================================
# VISUALIZAÇÃO DE RESULTADOS: Atendidos vs Não Atendidos (Spillovers - Maquinário 1920)
# ==============================================================================
...
df_resultados <- read_csv("03-resultados/csv/second_stage_1920_spillover_vizinhos_maquinario.csv", show_col_types = FALSE)

# 2. Manipulação de Dados (Filtragem e Renomeação)
df_plot <- df_resultados |>
  # Filtrar apenas os municípios atendidos e os vizinhos
...
# Para não sobrecarregar o eixo X, podemos filtrar para anos específicos 
# como décadas e anos notáveis (ex: 1870, 1880, 1890, 1900, 1910, 1920)
# Vamos pegar o final de cada década mais o primeiro e ultimo
anos_destaque <- c(min(df_plot$ano_outcome), seq(1870, 1920, by = 10), max(df_plot$ano_outcome))
# Garantir que existam na base (pegar o mais próximo se não existir)
...
# 8. Exportar
ggsave(
  filename = "03-resultados/plots/spillover_effects_maquinario_1920.png",
  plot = grafico_final,
  width = 12, 
  height = 5.5, 
  dpi = 300,
...
cat("Gráfico exportado: 03-resultados/plots/spillover_effects_maquinario_1920.png\n")
